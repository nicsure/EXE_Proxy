...
namespace EXE_Proxyfw
{
    ...
    internal class Program
    {
        ...
        {
            string exe = Environment.GetCommandLineArgs()[0]; // get the path+name of the exe
            if (exe.ToLower().EndsWith(".exe")) exe = exe.Substring(0, exe.Length - 4); // remove .exe from end of executable
            ...
            using (Process process = new Process()) // initialize a new process
            {
                ...
                process.StartInfo.UseShellExecute = false; // required to allow stream redirecting
